...
(defn step [n]
  (if (zero? n)
    1
    (let [c (inc (int (Math/floor (Math/log10 n))))]
      (if (even? c)
        (map (comp Integer/parseInt (partial apply str)) (split-at (/ c 2) (str n)))
        (* 2024)))))

(count
 (loop [state (map Integer/parseInt (re-seq #"\d+"
                                            ;; "125 17"
                                            (get-input "11")
                                            ))
        times 75]
   (if (zero? times)
     state
     (recur (mapcat step state) (dec times)))))
