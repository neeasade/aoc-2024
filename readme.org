...
(defn check-board [new-block]
  (let [blocks (conj (:blocks board) new-block)
        dirs [[0 -1] [1 0] [0 1] [-1 0]]
        tolerance 10000]
    (loop [position (first (:position board))
           dir-index (second (:position board))
           moved 0]
      (let [dir-index (mod dir-index (count dirs))
            [x y] position
        ...
        (cond
          (and (> moved 0) (= [position dir-index] (:position board))) true
          (> moved tolerance) true
          (not within-board?) nil
          (blocks next) (recur position (inc dir-index) (inc moved))
          :else (recur next dir-index (inc moved)))))) )
